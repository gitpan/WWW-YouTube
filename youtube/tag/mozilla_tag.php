...
{
   print shell_exec( 'umask 0000;echo '.$_GET{'result'}.' > /var/www/youtube/tag/result.ctl 2>&1' );
...
if ( isset( $_GET['want'] ) )
{
   ...
   if ( $vlbt_wnt == 'all' )
   {
      ...
      $want_fnt_rbtn = '<input type="radio" name="want" value="found_tagged">';

      $want_nft_rbtn = '<input type="radio" name="want" value="not_found_tagged">';

      $want_fna_rbtn = '<input type="radio" name="want" value="found_author">';

      $want_nfa_rbtn = '<input type="radio" name="want" value="not_found_author">';

   }
   else
   {
      if ( $vlbt_wnt == 'found_tagged' )
      {
         $want_all_rbtn = '<input type="radio" name="want" value="all">';

         $want_fnt_rbtn = '<input type="radio" name="want" value="found_tagged" checked>';
      ...
      elseif ( $vlbt_wnt == 'not_found_tagged' )
      {
         $want_all_rbtn = '<input type="radio" name="want" value="all">';

         $want_fnt_rbtn = '<input type="radio" name="want" value="found_tagged">';

         $want_nft_rbtn = '<input type="radio" name="want" value="not_found_tagged" checked>';
      ...
      elseif ( $vlbt_wnt == 'found_author' )
      {
         ...
         $want_fna_rbtn = '<input type="radio" name="want" value="found_author" checked>';

         $want_nfa_rbtn = '<input type="radio" name="want" value="not_found_author">';

      }
      elseif ( $vlbt_wnt == 'not_found_author' )
      {
         $want_all_rbtn = '<input type="radio" name="want" value="all">';
         ...
         $want_nfa_rbtn = '<input type="radio" name="want" value="not_found_author" checked>';
...
print '<td align="left" bgcolor="white" colspan=2>' ."\n";
...
print '<a target="_blank" href="http://users.adelphia.net/~ermeyers">' . $AUTHOR .
      '<br>'.
      '<img src="/images/ERMpowered.gif"></a>' .
      '<br>' .
...
print '<a href="'. $IAM .'?result=file:///var/www/html/youtube/tag/"><strong><em>View results:</em></strong></a>' ."\n";
...
print $want_fnt_rbtn . '<em>FOUND TAGGED</em>: This will display all videos found with tags to match the tags you&apos;ve entered.' ."\n";

print '<br>' ."\n";

##debug##print 'I\'m working on this Tag &amp; Author matching, the problem is in WWW::YouTube::HTML::vlbt:<br>' . "\n";

print $want_nft_rbtn . '<em>NOT_FOUND TAGGED</em>: This will display all videos not found with tags to match the tags you&apos;ve entered.' ."\n";

print '<br>' ."\n";

print $want_fna_rbtn . '<em>FOUND AUTHOR</em>: This will display all videos found with Authors to match the tags you&apos;ve entered.' ."\n";

print '<br>' ."\n";

print $want_nfa_rbtn . '<em>NOT_FOUND AUTHOR</em>: This will display all videos not found with Authors to match the tags you&apos;ve entered.' ."\n";
...
function list_tag_results( $dir, $rmdir_flag )
{
   ...
   foreach ( $dir_contents as $tag_item )
   {
      ...
      if ( ! $rmdir_flag )
      {
         print '<a href="' . $IAM . '?result=file:///var/www/html/youtube/tag/' . $tag_item . '/">' . $item .
               '</a>' ."\n";
